import { MessageSizeOption } from '../types/preferences.types';

export const MESSAGE_SIZE_KEY = 'chat_message_size';

export const MESSAGE_SIZE_OPTIONS: MessageSizeOption[] = [
  {
    value: 0,
    label: 'Tiny',
...
const normalizeMessageSize = (value: unknown): number => {
...
export const getMessageSize = (): number => {
  if (typeof window === 'undefined') return 3;
  const storedValue = localStorage.getItem(MESSAGE_SIZE_KEY);
  return normalizeMessageSize(storedValue);
};

export const saveMessageSize = (value: number | string): void => {
  const nextValue = normalizeMessageSize(value);
  localStorage.setItem(MESSAGE_SIZE_KEY, String(nextValue));
  window.dispatchEvent(
    new CustomEvent('message-preferences-change', {
      detail: { key: MESSAGE_SIZE_KEY, value: nextValue },
    })
  );
};

export const getMessageSizeOption = (value: unknown): MessageSizeOption => {

...
import { faDownload, faPlay, faPause, faXmark } from '@fortawesome/free-solid-svg-icons';
...
const WAVEFORM_BARS = [
  8, 14, 20, 11, 24, 17, 28, 13, 22, 31, 18, 10, 25, 16, 29, 21, 12, 26, 18, 32, 15, 23, 11, 27,
  19, 30, 14, 22, 9, 17,
];
...
const AudioPlayer: React.FC<AudioPlayerProps> = ({
  ...
  return (
    <div
      className={`${styles.audioPlayer} ${isVoice ? styles.audioPlayerVoice : ''} ${
        footer ? styles.audioPlayerWithFooter : ''
      }`}
            ...
            style={
              {
                '--audio-download-progress': `${
                  Math.max(0, Math.min(100, Number(downloadProgress) || 0)) * 3.6
                }deg`,
            ...
            <div className={styles.audioWaveform} aria-hidden="true">
              {WAVEFORM_BARS.map((height, index) => (
                <span
                  key={`${height}-${index}`}
                  className={
                    index / (WAVEFORM_BARS.length - 1) <= progress
                      ? styles.audioWaveformPlayed
                      : ''
                  }

...
import { Input, ProfileAvatar } from '@/shared/components';
...
export interface NewConversationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectUser?: (selectedUser: MemberInfo) => void;
}
    ...
    async (query = '') => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const allUsers: MemberInfo[] = data.members_info || data || [];
          ...
          <div className={styles.usersList}>
            {users.length === 0 ? (
              ...
              users.map((userItem) => (
                ...
                >
                  <ProfileAvatar size="md" src={userItem.profile_pic || defaultAvatar} />
                  <div className={styles.userInfo}>
                    <h3 className={styles.username}>{userItem.username}</h3>
                    {userItem.bio && <p className={styles.userBio}>{userItem.bio}</p>}
                  </div>
                </div>
              ))
            )}

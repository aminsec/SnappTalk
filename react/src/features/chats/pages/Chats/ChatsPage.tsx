...
function ChatsPage() {
  ...
  const promotePendingConversation = useCallback((tempId, realId) => {
    ...
    Object.values(pendingSendMapRef.current).forEach((pendingMessage: any) => {
      if (pendingMessage?.conversationId?.toString() === tempIdStr) {
        pendingMessage.conversationId = realIdStr;
      }
    });
    Object.values(pendingReplyMapRef.current).forEach((pendingMessage: any) => {
      if (pendingMessage?.conversationId?.toString() === tempIdStr) {
        pendingMessage.conversationId = realIdStr;
      }
    });
  ...
  const downloadMediaBlob = useCallback(async (message: any, preferredUrl = '', options: { keepLoading?: boolean } = {}) => {
  ...
  useEffect(() => () => {
    ...
    resolvedUrls.forEach((mediaUrl) => {
      if (typeof mediaUrl === 'string' && mediaUrl.startsWith('blob:')) URL.revokeObjectURL(mediaUrl);
    });
  ...
  useEffect(() => {
    ...
    socket.off(SOCKET_EVENTS.MESSAGE_RECEIVE_REPLY, handleMessageReceiveReply);
      ...
      Object.values(statusOfflineTimersRef.current).forEach((timeoutId) => clearTimeout(timeoutId as any));
      statusOfflineTimersRef.current = {};
      Object.values(pendingAckTimersRef.current).forEach((timeoutId) => clearTimeout(timeoutId as any));
  ...
  useEffect(() => {
    ...
    messages.forEach((msg: any) => {
      const id = getMessageId(msg)?.toString();
      if (id) messageById.set(id, msg);
    });
          ...
          if (!resolvedConversationId || resolvedConversationId === tempConversationId) {
            ...
            if (!mediaCreation) {
              ...
              pendingPvMediaCreationQueueRef.current = (waitForPreviousCreation
                .then(() => creationTurn) as Promise<any>);
  ...
  return (
    <div
      ...
      style={chatThemeStyle as React.CSSProperties}
    >

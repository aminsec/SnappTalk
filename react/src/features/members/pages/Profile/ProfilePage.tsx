...
import { faPaperPlane, faCalendarDays, faArrowLeft } from '@fortawesome/free-solid-svg-icons';
...
import { MemberInfo } from '@/shared/types/user.types';
...
const ProfilePage: React.FC = () => {
  ...
  const statusOfflineTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    ...
    const handleStatusOnline = (payload: { user_id?: string | number }) => {
    ...
    const handleStatusOffline = (payload: { user_id?: string | number }) => {
      const eventUserId = payload?.user_id?.toString();
      if (!eventUserId || eventUserId !== viewedUserId) {
        return;
      }
